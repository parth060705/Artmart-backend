...
# def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)) -> User:
#     """Return current logged-in user from JWT token (works for Google + normal login)."""
#     decoded = decode_access_token(token)
#     if not decoded or not decoded.get("user_id"):
#         raise HTTPException(
#             status_code=status.HTTP_401_UNAUTHORIZED,
...
#     # Prefer ID-based lookup (Google + normal login compatible)
...
#     db: Session = Depends(get_db)
# ):
#     """Return current user if logged in; otherwise None."""
#     if not token:
#         return None
