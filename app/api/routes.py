...
from app.schemas.schemas import UserCreate, UserRead, ArtworkCreate, ArtworkRead  # etc.
...
@router.get("/orders/user/{user_id}", response_model=List[schemas.OrderRead])
...
@router.get("/reviews/artist/{artist_id}", response_model=List[schemas.ReviewRead])
...
@router.post("/wishlist", response_model=schemas.WishlistRead)
def add_to_wishlist(item: schemas.WishlistCreate, db: Session = Depends(get_db)):
    return crud.add_to_wishlist(db, item)

@router.get("/wishlist/user/{user_id}", response_model=List[schemas.WishlistRead])
...
@router.get("/cart/user/{user_id}", response_model=List[schemas.CartRead])
